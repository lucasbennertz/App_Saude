...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _cadastrarUsuario() async {
    ...
    String dataNasc = _birthController.text.trim();
  ...
  Future<void> _loginUsuario() async {
    ...
    bool credenciaisCorretas = await database.verificarCredenciais(
      email,
      senha,
    );
    ...
    if (credenciaisCorretas) {
      ...
      if (user != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("Erro ao recuperar dados do usuário.")),
        );
      }
    } else {
      ScaffoldMessenger.of(
            ...
            child: Column(
              ...
              children: [
                ...
                _buildTextField(
                  "Email",
                  _emailController,
                  validator: (value) => regras.validarEmail(value!),
                ),
                SizedBox(height: 16),
                _buildTextField(
                  "Senha",
                  _passController,
                  obscureText: true,
                  validator: (value) => regras.validarSenha(value!),
                ),
                ...
                if (userCadastro) ...[
                  ...
                  _buildTextField(
                    "Nome",
                    _nameController,
                    validator: (value) => regras.validarNome(value!),
                  ),
                  SizedBox(height: 16),
                  Container(
                  ...
                  children: [
                    TextButton(
                      onPressed: () {
                        setState(() {
                          userCadastro =
                              !userCadastro; // Alterna entre login e cadastro
                        });
                      },
                      child: Text(
                        userCadastro
